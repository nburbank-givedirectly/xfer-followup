...
The most frequently selected expense categories vary substantially across projects. These differences partly stem from the distinct sets of expense categories provided for selection in follow-up surveys, which varied by project and timing.[^12] However, even among the core expense categories consistently included in all surveys, notable differences persist. For example, in the DRC Goma Large Transfer project, only 7.0% of recipients said they used their funds for food, in contrast to the (more typical) Rwanda Large Transfer program, where 72.4% of participants reported allocating their money to food purchases.

Using the higher-level expense categories, I calculated both the raw percentage of respondents who reported spending part or all of their transfer in each category and the inverse-weighted percentages across project type, year, country, age, and gender subgroups. These tables can be found in the `by_proj`, `by_country`, etc., tabs within the [project results](https://docs.google.com/spreadsheets/d/184txFsl9PnPbZav1lzhNk_th_r4OkiowhZepRBeaERQ/edit#gid=1154303096) spreadsheet, with the `_iw` suffix indicating an inverse weighted version of the analysis. The counts and percentages among the original (ungrouped) expense categories by project can also be found in [this spreadsheet](https://docs.google.com/spreadsheets/d/1sot14ktAlaI7vvfU9psVxnaJbSB-JP_uf3ahMxBdwe8/edit#gid=1692504833).
...
[^5]: The full mapping, including categories that were never selected in the past four years, is defined [here](https://github.com/nburbank-givedirectly/xfer-followup/blob/22fac762e351be90505d6100118df603485aeec9/src/mappings.py#L337).
...
[^12]: See for example the 48% of respondents from Liberia MoE Covid 19 emergency response program who reported spending a portion of their transfer on 'Transportation to school'. My understanding is that this is the only program where this expense category was offered. 
